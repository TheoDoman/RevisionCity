...
-- ── 9. Classes + memberships (teacher mode) ────────────────
...
CREATE POLICY "classes_insert_teacher"
  ON classes FOR INSERT WITH CHECK (teacher_user_id = auth.uid()::text);
...
CREATE POLICY "memberships_insert_self"
  ON class_memberships FOR INSERT WITH CHECK (student_user_id = auth.uid()::text);
